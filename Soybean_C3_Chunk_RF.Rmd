...
```{r}
soy.cmp <- read.csv(here("Data_Prediction/Compress_3_Chunk", "Soybean_X_C3_2.csv"), row.names = 1)
```
...
```{r}
saveRDS(y.out,
        here("ModelMetaData_Prediction/Net_3_RF_Chunk", "Soybean_PredictedOutput_C3_Chunk_2.rds"))
```
output data as csv
```{r}
write.csv(y.out, here("ModelMetaData_Prediction/Net_3_RF_Chunk",  "Soybean_PredictedOutput_C3_Chunk_2.csv"), quote = F)
```
...
```{r}
write.csv(y.predresult.df, here("ModelMetaData_Prediction/Net_3_RF_Chunk",  "Soybean_PredictionAccuracy_RF_C3_Chunk_2.csv"), quote = F)
```