...
```{r}
soy.cmp <- read.csv(here("Data_Prediction/Raw_Chunk", "Soybean_X_Raw_0.csv"), row.names = 1)
```
...
```{r}
saveRDS(y.out,
        here("ModelMetaData_Prediction/Raw_RF_Chunk", "Soybean_PredictedOutput_Raw_Chunk_0.rds"))
```
output data as csv
```{r}
write.csv(y.out, here("ModelMetaData_Prediction/Raw_RF_Chunk",  "Soybean_PredictedOutput_Raw_Chunk_0.csv"), quote = F)
```
...
```{r}
write.csv(y.predresult.df, here("ModelMetaData_Prediction/Raw_RF_Chunk",  "Soybean_PredictionAccuracy_RF_Raw_Chunk_0.csv"), quote = F)
```