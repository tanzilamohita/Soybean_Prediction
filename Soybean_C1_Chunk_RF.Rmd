...
```{r}
soy.cmp <- read.csv(here("Data_Prediction/Compress_1_Chunk", "Soybean_X_C1_8.csv"), row.names = 1)
```
...
```{r}
saveRDS(y.out,
        here("ModelMetaData_Prediction/Net_1_RF_Chunk", "Soybean_PredictedOutput_C1_Chunk_8.rds"))
```
output data as csv
```{r}
write.csv(y.out, here("ModelMetaData_Prediction/Net_1_RF_Chunk",  "Soybean_PredictedOutput_C1_Chunk_8.csv"), quote = F)
```
...
```{r}
write.csv(y.predresult.df, here("ModelMetaData_Prediction/Net_1_RF_Chunk",  "Soybean_PredictionAccuracy_RF_C1_Chunk_8.csv"), quote = F)
```